...
-- =============================================================
-- IDENTIDADE — substitui auth.uid() do Supabase.
-- A camada de API (Vercel), após validar o JWT do Clerk e resolver
-- o profiles.id correspondente, DEVE rodar por transação/request:
--   SELECT set_config('app.current_user_id', '<uuid>', true);
-- antes de qualquer query. Sem isso, current_user_id() retorna NULL
-- e toda política RLS abaixo nega acesso (fail-closed).
-- =============================================================
...
-- 2. profiles — identidade agora ancorada no Clerk, não em auth.users
...
CREATE TABLE IF NOT EXISTS equipe_convites (
  ...
  usado_por uuid        REFERENCES profiles(id) ON DELETE NO ACTION,
...
CREATE TABLE IF NOT EXISTS equipe_membros (
  id        uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  owner_id  uuid NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  membro_id uuid NOT NULL UNIQUE REFERENCES profiles(id) ON DELETE CASCADE,
  criado_em timestamptz DEFAULT now()
);
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT COALESCE(
    (SELECT is_admin FROM public.profiles WHERE id = app.current_user_id() LIMIT 1),
    FALSE
  );
$$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = app.current_user_id() AND is_admin = true) THEN
    RETURN NEW;
  END IF;
...
-- Antes (Supabase): trigger AFTER INSERT ON auth.users.
-- Agora: chamada explícita pela API a partir do webhook user.created do Clerk.
-- Combina o que eram duas triggers separadas (handle_new_user + agendar_sequencia_email).
CREATE OR REPLACE FUNCTION provision_new_user(
  p_clerk_user_id text, p_email text, p_nome text DEFAULT NULL,
  p_perfil text DEFAULT 'pintor', p_tel text DEFAULT NULL, p_cidade text DEFAULT NULL
)
RETURNS uuid LANGUAGE plpgsql VOLATILE SECURITY DEFINER AS $$
DECLARE
  v_id uuid;
BEGIN
  INSERT INTO public.profiles (clerk_user_id, email, nome, perfil, tel, cidade, trial_inicio)
  VALUES (
    p_clerk_user_id, p_email,
    COALESCE(p_nome, split_part(p_email, '@', 1)),
    COALESCE(p_perfil, 'pintor'), p_tel, p_cidade, now()
  )
  ON CONFLICT (clerk_user_id) DO UPDATE SET email = EXCLUDED.email
  RETURNING id INTO v_id;

  INSERT INTO public.email_queue (user_id, email, nome, template, agendado_para)
  VALUES (v_id, p_email, p_nome, 'boas_vindas',       now() + interval '2 minutes');
  INSERT INTO public.email_queue (user_id, email, nome, template, agendado_para)
  VALUES (v_id, p_email, p_nome, 'dica_d3',           now() + interval '3 days');
  INSERT INTO public.email_queue (user_id, email, nome, template, agendado_para)
  VALUES (v_id, p_email, p_nome, 'trial_d10',         now() + interval '10 days');
  INSERT INTO public.email_queue (user_id, email, nome, template, agendado_para)
  VALUES (v_id, p_email, p_nome, 'ultima_chance_d14', now() + interval '13 days 12 hours');
...
BEGIN
  UPDATE public.profiles SET conta_excluir_em = v_exclusao_em, ativo = false, atualizado_em = now() WHERE id = app.current_user_id();
  UPDATE public.subscriptions SET status = 'cancelada', atualizado_em = now() WHERE user_id = app.current_user_id() AND status = 'ativa';
  RETURN jsonb_build_object(
...
BEGIN
  ...
  WHERE id = app.current_user_id() AND conta_excluir_em IS NOT NULL AND conta_excluir_em > now();
END;
$$;
...
BEGIN
  UPDATE public.profiles SET lgpd_aceito = true, lgpd_aceito_em = now(), lgpd_versao = p_versao,
    termos_aceitos_em = now(), atualizado_em = now() WHERE id = app.current_user_id();
END;
$$;
...
BEGIN
  ...
  IF NOT FOUND THEN RETURN jsonb_build_object('ok', false, 'erro', 'Convite inválido ou expirado.'); END IF;
  IF v_convite.owner_id = app.current_user_id() THEN RETURN jsonb_build_object('ok', false, 'erro', 'Você é o dono desta equipe.'); END IF;
  IF EXISTS (SELECT 1 FROM equipe_membros WHERE membro_id = app.current_user_id()) THEN
  ...
  INSERT INTO equipe_membros (owner_id, membro_id) VALUES (v_convite.owner_id, app.current_user_id()) ON CONFLICT (membro_id) DO NOTHING;
  UPDATE equipe_convites SET usado_por = app.current_user_id(), usado_em = now() WHERE id = v_convite.id;
  UPDATE profiles SET plano = v_owner_plano, equipe_owner_id = v_convite.owner_id WHERE id = app.current_user_id();
...
BEGIN
  SELECT plano INTO v_plano FROM profiles WHERE id = app.current_user_id();
  ...
  SELECT COUNT(*) INTO v_membros FROM equipe_membros WHERE owner_id = app.current_user_id();
  IF v_membros >= v_lim THEN RAISE EXCEPTION 'Limite de % membros atingido', v_lim; END IF;
  DELETE FROM equipe_convites WHERE owner_id = app.current_user_id() AND usado_por IS NULL;
  INSERT INTO equipe_convites (owner_id) VALUES (app.current_user_id()) RETURNING codigo INTO v_codigo;
  RETURN v_codigo;
END;
$$;
...
RETURNS jsonb LANGUAGE plpgsql VOLATILE SECURITY DEFINER AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM profiles WHERE id = app.current_user_id() AND plano IN ('equipe', 'ia-pro')) THEN
    RETURN jsonb_build_object('ok', false, 'erro', 'Sem permissão.');
  END IF;
  UPDATE equipe_convites SET usado_por = app.current_user_id(), usado_em = now() WHERE owner_id = app.current_user_id() AND usado_por IS NULL;
  RETURN jsonb_build_object('ok', true);
END;
$$;
...
RETURNS jsonb LANGUAGE plpgsql VOLATILE SECURITY DEFINER AS $$
DECLARE
  v_uid uuid := app.current_user_id(); v_plano text; v_result jsonb;
BEGIN
  SELECT plano INTO v_plano FROM profiles WHERE id = v_uid;
  ...
  SELECT jsonb_agg(jsonb_build_object('membro_id', em.membro_id, 'nome', COALESCE(p.nome,'—'),
    'email', COALESCE(p.email,'—'), 'criado_em', em.criado_em) ORDER BY em.criado_em)
...
RETURNS jsonb LANGUAGE plpgsql VOLATILE SECURITY DEFINER AS $$
DECLARE
  v_uid uuid := app.current_user_id(); v_plano text; v_count int; v_codigo text; v_owner_id uuid; v_dono_nome text;
BEGIN
  SELECT plano INTO v_plano FROM profiles WHERE id = v_uid;
...
BEGIN
  v_usuario_id := app.current_user_id();
...
BEGIN
  ...
  -- Envio de e-mails da fila NÃO é mais disparado daqui (net.http_post era específico
  -- do Supabase). Um cron job da Vercel (vercel.json "crons") deve chamar uma rota
  -- /api/cron/email-sender que processa public.email_queue diretamente.

  RETURN jsonb_build_object('executado_em', agora, 'trials_rebaixados', n_trials,
    'assinat_expiradas', n_assinat, 'laudos_resetados', n_reset_laudos);
END;
$$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM profiles WHERE id = app.current_user_id() AND perfil = 'admin') THEN
    RAISE EXCEPTION 'Acesso negado: permissão de admin necessária.';
  END IF;
  RETURN QUERY
  SELECT p.id, p.nome, p.email, p.plano,
...
-- planos_config
CREATE POLICY "planos_config_leitura_publica" ON planos_config FOR SELECT USING (true);
CREATE POLICY "planos_config_admin_write"     ON planos_config FOR ALL    USING (is_admin()) WITH CHECK (is_admin());

-- profiles
CREATE POLICY "profiles: leitura"          ON profiles FOR SELECT      USING ((app.current_user_id() = id) OR is_admin());
CREATE POLICY "profiles: insercao"         ON profiles FOR INSERT WITH CHECK (app.current_user_id() = id);
CREATE POLICY "profiles: atualizacao usuario" ON profiles FOR UPDATE USING (app.current_user_id() = id) WITH CHECK (app.current_user_id() = id);
CREATE POLICY "profiles: atualizacao admin"   ON profiles FOR UPDATE USING (is_admin()) WITH CHECK (is_admin());
CREATE POLICY "profiles: admin ve todos"      ON profiles FOR ALL    USING (is_admin()) WITH CHECK (is_admin());

-- afiliados
CREATE POLICY "user_own_afiliados" ON afiliados FOR ALL USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);

-- comissoes
CREATE POLICY "user_own_comissoes"        ON comissoes FOR SELECT USING (afiliado_id IN (SELECT id FROM afiliados WHERE user_id = app.current_user_id()));
CREATE POLICY "comissoes_no_direct_write" ON comissoes FOR INSERT WITH CHECK (false);

-- pagamentos
CREATE POLICY "pagamentos: usuario le proprios" ON pagamentos FOR SELECT      USING (app.current_user_id() = user_id);
CREATE POLICY "pagamentos: admin total"         ON pagamentos FOR ALL    USING (is_admin()) WITH CHECK (is_admin());

-- orcamentos
CREATE POLICY "orcamentos_proprio"              ON orcamentos FOR ALL    USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);
CREATE POLICY "orcamentos: public read by token" ON orcamentos FOR SELECT USING ((sig_token IS NOT NULL) AND ((sig_token_expires_at IS NULL) OR (sig_token_expires_at > now())));
CREATE POLICY "orcamentos: admin ve todos"       ON orcamentos FOR SELECT USING (is_admin());
CREATE POLICY "admin_all_orcamentos"             ON orcamentos FOR ALL    USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = app.current_user_id() AND profiles.perfil = 'admin'));
CREATE POLICY "equipe_owner_ve_orcamentos"       ON orcamentos FOR SELECT USING (EXISTS (SELECT 1 FROM equipe_membros WHERE equipe_membros.owner_id = app.current_user_id() AND equipe_membros.membro_id = orcamentos.user_id));

-- contratos
CREATE POLICY "contratos_owner"              ON contratos FOR ALL    USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);
CREATE POLICY "contratos_public_read"        ON contratos FOR SELECT USING ((sig_token IS NOT NULL) AND ((sig_token_expires_at IS NULL) OR (sig_token_expires_at > now())));
CREATE POLICY "equipe_owner_ve_contratos"    ON contratos FOR SELECT USING (EXISTS (SELECT 1 FROM equipe_membros WHERE equipe_membros.owner_id = app.current_user_id() AND equipe_membros.membro_id = contratos.user_id));

-- laudos
CREATE POLICY "laudos: usuario le os proprios"      ON laudos FOR SELECT USING (app.current_user_id() = user_id);
CREATE POLICY "laudos: usuario insere os proprios"  ON laudos FOR INSERT WITH CHECK (app.current_user_id() = user_id);
CREATE POLICY "laudos: usuario atualiza os proprios" ON laudos FOR UPDATE USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);
CREATE POLICY "laudos: usuario deleta os proprios"  ON laudos FOR DELETE USING (app.current_user_id() = user_id);
CREATE POLICY "laudos: public read by token"        ON laudos FOR SELECT USING ((sig_token IS NOT NULL) AND ((sig_token_expires_at IS NULL) OR (sig_token_expires_at > now())));
CREATE POLICY "laudos: admin ve todos"              ON laudos FOR SELECT USING (is_admin());
CREATE POLICY "admin_all_laudos"                    ON laudos FOR ALL    USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = app.current_user_id() AND profiles.perfil = 'admin'));
CREATE POLICY "equipe_owner_ve_laudos"              ON laudos FOR SELECT USING (EXISTS (SELECT 1 FROM equipe_membros WHERE equipe_membros.owner_id = app.current_user_id() AND equipe_membros.membro_id = laudos.user_id));

-- recibos
CREATE POLICY "recibos_owner"           ON recibos FOR ALL    USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);
CREATE POLICY "equipe_owner_ve_recibos" ON recibos FOR SELECT USING (EXISTS (SELECT 1 FROM equipe_membros WHERE equipe_membros.owner_id = app.current_user_id() AND equipe_membros.membro_id = recibos.user_id));

-- documentos_assinados
CREATE POLICY "pintor_ve_suas_assinaturas" ON documentos_assinados FOR ALL USING (app.current_user_id() = usuario_id);

-- email_queue
CREATE POLICY "email_queue_user_read"        ON email_queue FOR SELECT USING (app.current_user_id() = user_id);
CREATE POLICY "email_queue_no_direct_insert" ON email_queue FOR INSERT WITH CHECK (false);

-- empresa_config
CREATE POLICY "empresa_config_proprio" ON empresa_config FOR ALL USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);

-- equipe_convites
-- NOTA: "equipe_convites_leitura" (USING true, qualquer autenticado lê qualquer convite)
-- existia em schema-base.sql mas NUNCA esteve ativa em produção (confirmado via
-- pg_policies em 2026-07-12) e é um vazamento cross-tenant — não recriada aqui de propósito.
CREATE POLICY "equipe_convites_owner"   ON equipe_convites FOR ALL    USING (owner_id = app.current_user_id()) WITH CHECK (owner_id = app.current_user_id());
CREATE POLICY "convite_pub_v2"          ON equipe_convites FOR SELECT USING ((ativo = true) AND (expira_em > now()) AND (usado_por IS NULL));
CREATE POLICY "owner_convite_v2"        ON equipe_convites FOR ALL    USING (owner_id = app.current_user_id());

-- equipe_membros
CREATE POLICY "equipe_membros_owner"       ON equipe_membros FOR ALL    USING (owner_id = app.current_user_id()) WITH CHECK (owner_id = app.current_user_id());
CREATE POLICY "equipe_membros_membro_read" ON equipe_membros FOR SELECT USING (membro_id = app.current_user_id());
CREATE POLICY "ver_membros_v2"             ON equipe_membros FOR SELECT USING ((owner_id = app.current_user_id()) OR (membro_id = app.current_user_id()));

-- equipes
CREATE POLICY "equipes_proprio" ON equipes FOR ALL USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);

-- templates
CREATE POLICY "templates_proprio" ON templates FOR ALL USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);

-- agenda
CREATE POLICY "agenda_proprio"          ON agenda FOR ALL    USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);
CREATE POLICY "equipe_owner_ve_agenda"  ON agenda FOR SELECT USING (EXISTS (SELECT 1 FROM equipe_membros WHERE equipe_membros.owner_id = app.current_user_id() AND equipe_membros.membro_id = agenda.user_id));

-- despesas
CREATE POLICY "despesas_proprio" ON despesas FOR ALL USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);

-- obras
CREATE POLICY "obras_proprio" ON obras FOR ALL USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);

-- eventos
CREATE POLICY "eventos_owner" ON eventos FOR ALL USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);

-- propostas_publicas
-- NOTA: "propostas_leitura_publica" (USING true) existia em schema-base.sql mas foi
-- corretamente revogada em produção em 2026-05-31 (vazamento público total) — não
-- recriada aqui. Adicionado no lugar: dono lendo os próprios registros (faltava até
-- em produção; corrigido em 2026-07-12 via achado da auditoria).
CREATE POLICY "propostas_owner_select"    ON propostas_publicas FOR SELECT USING (app.current_user_id() = user_id);
CREATE POLICY "propostas_insert_proprio"  ON propostas_publicas FOR INSERT WITH CHECK (app.current_user_id() = user_id);
CREATE POLICY "propostas_update_proprio"  ON propostas_publicas FOR UPDATE USING (app.current_user_id() = user_id) WITH CHECK (app.current_user_id() = user_id);

-- rate_limits (RESTRICTIVE — bloqueia acesso direto, só via RPC SECURITY DEFINER)
CREATE POLICY "rate_limits_no_direct_access" ON rate_limits AS RESTRICTIVE FOR ALL USING (false);
...
-- =============================================================
-- Provisionamento de novo usuário: NÃO é mais trigger em auth.users
-- (essa tabela não existe no Neon). A API (rota que recebe o webhook
-- user.created do Clerk) deve chamar:
--   SELECT provision_new_user('<clerk_user_id>', '<email>', '<nome>', ...);
-- =============================================================
