...
async function salvarContrato(existingId) {
  ...
  const uid = USER?.id;
  if (uid && uid !== 'demo') {
    const { error } = await sb.from('contratos').upsert({ ...obj, user_id: uid }, { onConflict: 'id' });
    if (error) { console.error('[MestrePro] Erro ao salvar contrato:', error.message); notif('❌ Erro ao salvar contrato. Tente novamente.'); return; }
  }
...
function renderAgenda() {
  ...
  for (let d = 1; d <= diasNoMes; d++) {
    ...
    const evs = _eventos.filter(e => {
      ...
      // Forçar T12:00:00 para evitar bug de timezone (UTC midnight → dia anterior em UTC-3)
      const iniStr = (e.data_inicio || e.inicio || '').split('T')[0];
      const fimStr = (e.data_fim || e.fim || '').split('T')[0];
      const ini = new Date(iniStr + 'T12:00:00');
      const fim = fimStr ? new Date(fimStr + 'T12:00:00') : ini;
      const dDate = new Date(dStr + 'T12:00:00');
      return dDate >= ini && dDate <= fim;
    });
  ...
  const upcoming = _eventos
    .filter(e => {
      // Forçar T12:00:00 para evitar bug de timezone — eventos de hoje somem sem isso
      const dStr2 = (e.data_inicio || e.inicio || '').split('T')[0];
      if (!dStr2) return false;
      const d = new Date(dStr2 + 'T12:00:00');
...
async function salvarEvento(existingId) {
  ...
  const uid = USER?.id;
  if (uid && uid !== 'demo') {
    const { error } = await sb.from('agenda').upsert({ ...obj, user_id: uid }, { onConflict: 'id' });
    if (error) { console.error('[MestrePro] Erro ao salvar evento:', error.message); notif('❌ Erro ao salvar evento. Tente novamente.'); return; }
  }
...
async function excluirEvento(id) {
  ...
  if (uid && uid !== 'demo') {
    ...
    if (error) { console.error('[MestrePro] Erro ao excluir evento:', error.message); notif('❌ Erro ao excluir evento. Tente novamente.'); return; }
